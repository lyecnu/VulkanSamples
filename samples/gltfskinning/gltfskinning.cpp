	...
	if (inputNode.mesh > -1)
	{
		...
		for (size_t i = 0; i < mesh.primitives.size(); ++i)
		{
			...
			// Vertices
			{
				...
				{
					Vertex vertex = {};
					vertex.pos = glm::make_vec3(&positionsBuffer[v * 3]);
...
void VulkanglTFModel::updateJoints(Node* node)
{
	if (node->skin > -1)
	{
...
VulkanExample::VulkanExample()
{
	...
	camera.setRotation(glm::vec3(0.0f, 0.0, 0.0f));
...
void VulkanExample::loadglTFFile(std::string filename)
{
	...
	glTFModel.indices.count = static_cast<int>(indexBuffer.size());
...
void VulkanExample::setupDescriptors()
{
	...
	const uint32_t maxSetCount = static_cast<uint32_t>(glTFModel.images.size() + glTFModel.skins.size() + 1);
	VkDescriptorPoolCreateInfo descriptorPoolCI = vks::initializers::descriptorPoolCreateInfo(poolSizes, maxSetCount * maxConcurrentFrames);
...
void VulkanExample::preparePipelines()
{
	...
	{
		loadShader(getShadersPath() + "gltfskinning/gltfskinning.vert.spv", VK_SHADER_STAGE_VERTEX_BIT),
		loadShader(getShadersPath() + "gltfskinning/gltfskinning.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT)
	};
